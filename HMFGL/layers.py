...
class OutputLayer2(nn.Module):
    def __init__(self, d_in, d_hidden, n_classes, modal_num, dropout = 0.5,dataname="TADPOLE"):
        ...
        # self.classifier = nn.Linear(d_hidden + modal_num**2, n_classes)
        if dataname == "ABIDE":
            self.classifier_attn = nn.Linear(16, n_classes)
        else:
            self.classifier_attn = nn.Linear(36, n_classes)
        self.classifier_x = nn.Linear(d_hidden, n_classes)
    ...
    def forward(self, x, attn_embedding):
        ...
        # output = self.classifier(combined_x)
        # return F.log_softmax(output, dim=1), combined_x
        output_x = self.classifier_x(x)
        output_attn = self.classifier_attn(attn_embedding)
        return F.log_softmax(output_x, dim=1), F.log_softmax(output_attn, dim=1), combined_x
