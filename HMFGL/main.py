...
def train_and_eval(datadir, datname, hyperpm):
    ...
    input_data = data[:, :-1]
    label = data[:, -1] - 1
    skf = StratifiedKFold(n_splits=12, random_state=hyperpm.seed, shuffle=True)
    set_rng_seed(hyperpm.seed)
    val_acc, tst_acc, tst_auc, tst_f1,tst_spe,tst_sen = [], [], [], [],[],[]
    ...
    for train_index, test_index in skf.split(input_data, label):
        ...
        for t in range(hyperpm.nepoch):
            ...
            tm = time.time()
            init_memory = torch.cuda.memory_allocated(device=dev)
            ...
            current_memory = torch.cuda.memory_allocated(device=dev) - init_memory
            current_memory = current_memory / 1024
            epoch_resources.append(current_memory)
        ...
        cur_tst_acc, cur_tst_auc, conf_mat,cur_tst_f1,cur_tst_spe,cur_tst_sen = agent.print_tst_acc(hyperpm.mode)


        tst_acc.append(cur_tst_acc)
        tst_auc.append(cur_tst_auc)
        tst_f1.append(cur_tst_f1)
        tst_spe.append(cur_tst_spe)
        tst_sen.append(cur_tst_sen)
    ...
    return np.array(val_acc).mean(), np.array(tst_acc).mean(), np.array(tst_acc).std(), \
           np.array(tst_auc).mean(), np.array(tst_auc).std(), \
           np.array(tst_f1).mean(), np.array(tst_f1).std(), \
           np.array(tst_spe).mean(), np.array(tst_spe).std(), \
           np.array(tst_sen).mean(), np.array(tst_sen).std()
           # np.array(epoch_times).mean(), np.array(epoch_resources).mean()
...
def main(args_str=None):
    ...
    # parser.add_argument('--kNN_sparsification',  default=True,
    #                     help='kNN_sparsification')
    # parser.add_argument('--adj_origin', default=True,
    #                     help='Constructing adjacency matrix using raw data')
    parser.add_argument('--Denoising', default=True,
                        help='DropEdge')
    parser.add_argument('--lsp', default=True,
                        help='Modify the LSP loss function')
    parser.add_argument('--lg', default=True,
                        help='Remove the loss function lg')
    parser.add_argument('--kNum', type=int,default=40,
                        help='The number of k-sparsing')
    ...
    with RedirectStdStreams(stdout=sys.stderr):
        ...
        val_acc, tst_acc, tst_acc_std, tst_auc, tst_auc_std, tst_f1, tst_f1_std,tst_spe, tst_spe_std,tst_sen, tst_sen_std = train_and_eval(args.datadir, args.datname, args)
        print('val=%.2f%% tst_acc=%.2f%% tst_auc=%.2f%% tst_f1=%.2f tst_spe=%.2f tst_sen=%.2f' % (val_acc * 100, tst_acc * 100, tst_auc * 100,tst_f1 * 100,tst_spe * 100,tst_sen * 100))
        print('tst_acc_std=%.4f tst_auc_std=%.4f tst_f1_std=%.4f tst_spe_std=%.4f tst_sen_std=%.4f' % (tst_acc_std, tst_auc_std,tst_f1_std,tst_spe_std,tst_sen_std))
